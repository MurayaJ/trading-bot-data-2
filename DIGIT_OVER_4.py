...
class DigitOver4(TradingAlgorithm):
    ...
    def get_features(self):
        ...
        P_over, P_not_over = self.predict_one_step(d_t, d_t_minus_1)
        indicators = self.df.iloc[-1][['MA_6', 'RSI', 'Volatility', 'Hour_sin', 'Hour_cos']].values
        return list(recent_digits) + [P_over, P_not_over] + list(indicators)
    ...
    def predict_one_step(self, d_t, d_t_minus_1=None):
        ...
        P_over = np.sum(p_next[5:])  # Probability for digits 5-9
        P_not_over = 1 - P_over
        return P_over, P_not_over
    ...
    def process_message(self, ws, data):
        ...
        if data.get('msg_type') == 'authorize':
            ...
            if len(self.digit_history) >= 3:
                ...
                if features:
                    target = 1 if last_digit > 4 else 0
            ...
            if self.training_samples >= 100 and not self.is_trading and (time.time() - self.last_trade_time) >= self.trade_cooldown and self.cumulative_profit < self.target_profit:
                ...
                if features:
                    ...
                    if rf_pred[1] > rf_threshold and P_over > 0.60:
                        self.buy_contract(ws, "DIGITOVER")
